<template>
    <div class="hidden sm:block relative flex-auto mx-[19px] bg-[#14151a] text-[#d7d4d4]">
        <div class="main">
            <div class="main-content w-full relative">
                <div class="w-full tabs border-b border-[#d7d4d4] ">
                    <div
                        class="text-base text-center pt-5 pb-1 cursor-pointer w-[180px] tracking-wider border-b-[3px] border-[#d7d4d4]">
                        {{ editAdd ? '修改取款账户' : '添加取款账户' }}
                    </div>
                </div>
                <!-- first step -->
                <section v-if="!verify" class="relative w-full bg-[#14151a] my-2 py-[20px] px-[18px]">
                    <div class="absolute left-5 top-5 h-5 w-12">
                        <img src="@/assets/mobile/user/num11.png" alt="" class="w-auto h-auto" loading="lazy">
                    </div>
                    <div class="py-5 flex flex-col justify-center items-center">
                        <div class="mb-[40px] flex space-x-3 items-center justify-center">
                            <div class=" py-2 rounded-full bg-slate-600 px-2 border-[#7e7e7e]">
                                ...
                                <img src="@/assets/Phone.png" alt="" class="w-10 h-10">
                            </div>
                        ...
                        <section class="form w-[381px]">
                            ...
                            <div
                                ...
                                <div
                                    class=" border-none w-full h-full text-sm flex justify-center items-center relative">
                                    <input v-model="phonecode" autocomplete="off"  id="phonecode" pattern="" 
                                        class=" input-name placeholder-[#d7d4d4] border-none outline-none focus:outline-none focus:border-none w-full h-full" 
                                        name="phonecode" placeholder="6位短信校验码" type="text" maxlength="6" min="6" max="6">
                                    <XIcon v-if="phonecode" @click="phonecode = ''"
                                        class="h-5 w-5 cursor-pointer absolute right-28 text-[#d7d4d4] ">
                                    </XIcon>
                ...
                <section v-if="verify" class="relative w-full my-2 py-[20px] px-[18px] text-[#d7d4d4]">
                    <div class="absolute left-8 top-5 h-5 w-12">
                    ...
                    <div class="py-5 flex flex-col justify-center items-center">
                        ...
                        <section class="form w-[381px]">
                            <div class="bg-[#27282d] my-3 formDiv relative overflow-hidden flex items-center mx-auto  h-14">
                                <div class=" border-none w-full h-full text-sm flex justify-center items-center relative">
                                    <input v-model="bankcard" autocomplete="off" id="bankcard" pattern="" 
                                        class=" input-name placeholder-[#d7d4d4] border-none outline-none focus:outline-none focus:border-none w-full h-full" 
                                        name="bankcard" placeholder="开户行名称" type="text" maxlength="17" min="6" max="6">
                                    <XIcon v-if="bankcard" @click="bankcard = ''"
                                        class="h-5 w-5 cursor-pointer  absolute right-5 text-[#d7d4d4] ">
                                    </XIcon>
                            ...
                            <div
                                ...
                                <div class=" border-none w-full h-full text-sm flex justify-center items-center relative">
                                    <input v-model="cardnumber" autocomplete="off" id="cardnumber" pattern=""
                                        class=" input-name placeholder-[#d0d0d0] border-none outline-none focus:outline-none focus:border-none w-full h-full" 
                                        name="cardnumber" placeholder="银行卡号" type="text" maxlength="17" min="6" max="6">
                                    <XIcon v-if="cardnumber" @click="cardnumber = ''"
                                        class="h-5 w-5 cursor-pointer  absolute right-5 text-[#d0d0d0] ">
                                    </XIcon>
                            ...
                            <div
                                ...
                                <div class=" border-none w-full h-full text-sm flex justify-center items-center relative">
                                    <span class="absolute left-0 px-3">手机号</span>
                                    <input disabled :value="hideWord(userValue?.realname)" autocomplete="off"  type="text"
                                        class=" input-name placeholder-[#d0d0d0] border-none outline-none focus:outline-none focus:border-none pl-16 w-full h-full text-sm">
                                </div>
                            </div>
                            <p class="text-red-500 text-sm py-3">请使用户名 提*的名字进行添加取款户 名字不相同则无非添加。如需更改名字 请联系在线客服 客服地址</p>
                            <div @click="editAdd ? editBankCard() : addBankCard()" class="relative cursor-pointer py-5">
                                <img src="@/assets/pc/Home/bg.png" alt="" class="w-auto h-auto">
                ...
                <section v-if="moneyAddData?.length > 0 && verify"
                    class=" w-full mt-[12px] py-[20px] px-[18px] relative bg-[#1f2129] text-[#d0d0d0]" >
                    <div class="flex items-center justify-between pb-[11px] border-b border-[#282b30]">
                        <div class=" flex items-center space-x-2 ">
                            <img src="@/assets/pc/Home/depo11.webp" alt="" class="w-6">
                            <p>添加银行卡</p>
                        </div>
                        <div class="text-[13px]">
                            数量 ({{ moneyAddData?.length || 0 }} / 5)
                        </div>
                    </div>
                    <section class=" w-full">
                        <div class=" mb-2 px-5 w-full flex flex-wrap justify-start   py-2">
                            <div v-for="bank in moneyAddData" :key="bank.id"
                                class="flex  bg-[#27282d] py-2 mr-3 mb-3 px-2 items-center w-[300px] justify-between">
                                <div class=" space-x-2">
                                    <span> {{ bank?.ma_address }}</span>
                                    <span>({{ hideWord(bank?.ma_id) }})</span>
                                </div>
                                <div @click="editAddress(bank)" class="text-red-600 cursor-pointer tracking-wider ">编辑
                                </div>
                            </div>
