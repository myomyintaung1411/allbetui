...
<template>
    <div class="sm:hidden relative  h-full overflow-y-auto w-full  text-[#d7d4d4] bg-[#14171e]">
        <div class="py-2 w-full relative flex items-center px-6 justify-between">
        ...
        <section class="px-3 w-full relative">
            <div class="user_bg bg-gradient-to-r from-[#1d1b1c] to-[#350d49] relative flex flex-col py-1  px-3">
                <div>
                    <div class=" first-letter:text-red-700 tracking-[3px]">{{ userValue?.username }}
                    </div>
                    <div class="flex relative items-center space-x-3">
                        <div class="w-20 relative h-7 ">
                            <img src="@/assets/mobile/user/level.png" alt="" class="w-full h-7">
                            <div class=" text-right absolute top-1 right-2 w-full h-7 text-black text-base ">
                ...
                <div class="flex items-center relative  mt-8">
                    <div v-if="activeTab == 1" class="flex justify-start items-center">
                        <img src="@/assets/mobile/user/money.png" alt="" class="w-6 h-6" />
                        <div class="text-base pt-1 px-3 text-white">{{ userValue?.balance || 0 }}</div>
                    </div>
                    <div v-else class="flex justify-start items-center">
                        <img src="@/assets/mobile/user/coin.png" alt="" class="w-6 h-6">
                        <div class="text-base pt-1 px-3 text-white">{{ userValue?.coin || 0 }}</div>
                    </div>
                    ...
                    <div @click="userDetail()" class="absolute right-0  flex justify-start items-center space-x-1">
                        <img src="@/assets/mobile/user/more.png" alt="" class="w-auto h-4">
                        <!-- <img src="@/assets/mobile/user/rightArrow.png" alt="" class="w-2 h-4"> -->
                    </div>
                </div>
                <div class="bg-[#d1970b] h-[1px] w-[150px] mt-2"></div>
            </div>
        </section>
        ...
        <section class="px-2 w-full relative py-3">
            <router-link to="/upgrade" class="flex px-3 justify-between items-center text-[#7c7a7a] text-[12px]">
                <div class="flex justify-start items-center space-x-1 text-[12px]">
                    <img src="@/assets/mobile/user/throphy.png" alt="" class="w-8 h-8">
                    <div class=" tracking-[3px] text-base px-1 text-[#d7d4d4]">会员晋级</div>
                    每周一更新星级
                </div>
                <div class="flex justify-end items-center">
                    <p>等级福利</p>
                    <ChevronRightIcon class="w-5 h-6 "></ChevronRightIcon>
                </div>
            </router-link>
            <div class="bg_champion my-4 px-2 flex items-center">
                <div class="flex items-center">
                    <img src="@/assets/mobile/user/star.png" alt="" class="w-6 h-6">
                    <div class="rule px-2 tracking-[3px]">每周一更新星级</div>
                    <div class="level-label text-[#953d38]">
                        升级
        ...
        <section class="pt-5 w-full">
            <div class="flex w-full items-start justify-evenly">
                <div @click="goSetting(1)">
                    <img src="@/assets/mobile/user/1111.png" alt="" class="w-[50px] h-[50px]">
                </div>
                <div @click="goSetting(2)">
                    <img src="@/assets/mobile/user/2222.png" alt="" class="w-[50px] h-[50px]">
                </div>
                <div @click="goSetting(3)">
                    <img src="@/assets/mobile/user/3333.png" alt="" class="w-[50px] h-[50px]">
                </div>
            </div>
            <div class="flex w-full items-start justify-between py-10 pl-14 pr-8">
                <div @click="goSetting(4)">
                    <img src="@/assets/mobile/user/6666.png" alt="" class="w-[48px] h-[50px]">
                </div>

                <div @click="goSetting(6)">
                    <img src="@/assets/mobile/user/7777.png" alt="" class="w-[59px] h-[52px]">
                </div>
                <div @click="goSetting(5)" class="w-[24%]">
                    <img src="@/assets/mobile/user/9999.png" alt="" class="w-[28px] h-[50px]">
                </div>
            </div>
            <!-- <div @click="goSetting(7)" v-if="token"
                class="w-full pb-5 cursor-pointer mx-9 relative">
                <img src="@/assets/logout.png" alt="" class="w-[19rem] h-[3rem]">
                <img src="@/assets/logout1.png" alt="" class="w-auto h-auto absolute top-0 flex justify-center items-center">
            </div> -->
            <div @click="goSetting(7)" v-if="token" class="relative cursor-pointer mx-14">
                <!-- <img src="@/assets/pc/Home/bg.png" alt="" class="w-[18rem] h-[3rem]">
                <div class="absolute bottom-0  w-full h-full flex justify-center items-center text-sm text-black">退出当前账户</div> -->
                <div class="w-[18rem] h-[3rem] flex justify-center items-center text-sm text-black btn">退出当前账户</div>
            </div>
        </section>
    </div>
    ...
    <NotCloseDialog :show="logoutDialog" @close="logoutDialog = false">
        <div class="w-[90vw] py-10  px-3">
            ...
            <div class="w-full text-start py-3">
                <p class="font-bold text-md text-center tracking-widest">您确定要退出账号吗？</p>
            </div>

            <div class="flex pt-5 items-center w-full  justify-evenly px-3">
                <router-link to="/" class="flex items-center justify-center h-[50px] border border-solid border-[#d1970b] text-[#d7d4d4] font-bold text-md cursor-pointer px-12">
                    取消
                </router-link>
                <div @click="logout" class="btn flex cursor-pointer px-12 items-center justify-center text-[#101114] font-bold text-md text-center h-[50px]">
                    确认
                </div>
            </div>
        </div>
...
.btn {
   /* @apply bg-gradient-to-t from-[#aca8a8] via-[#fdf4f4] to-[#aca8a8]; */
