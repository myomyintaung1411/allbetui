...
// instance.photos = axios.create({
//   baseURL: BaseUrl, // 接口统一域名
//   timeout: 6000, // 设置超时
//   withCredentials: true,
//   headers:{'Content-Type': 'application/json;charset=UTF-8'}
// })
// const instance_pay = axios.create({
//   baseURL: BaseUrl, // 接口统一域名
//   timeout: 6000, // 设置超时
//   withCredentials: true,
//   headers:{'Content-Type': 'application/json;charset=UTF-8'}
...
instance.interceptors.request.use((config) => {
  //const token = window.localStorage.getItem('tttt')
  const token = window.localStorage.getItem('tttt') ?
    JSON.parse(aes.Cryptodecrypt(JSON.parse(window.localStorage.getItem('tttt')), {
  ...
  console.log('ddddddddddddd ', config)
...
// instance_pay.interceptors.request.use((config) => {
//   const token = window.localStorage.getItem('tttt')
//   token && (config.headers.Authorization = `Bearer ${token}`)
//    if (config.method === 'post') {
//    }
...
// instance_pay.interceptors.response.use((response) => {
//   return response
// }, (error) => {
//   // 响应错误 Authorization token expired
