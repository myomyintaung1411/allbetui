        ...
        <div class="py-5 w-full">
            <form @click.prevent>
                <div class="bg-[#13151b] text-base relative">
                    ...
                    <div
                        ...
                        <div class="formInput w-full h-full text-sm flex justify-center items-center relative">
                            <input v-model.trim="phonenum" autocomplete="off" type="text"
                ...
                <div class="py-7 w-full px-5">
                    <button @click="binePhone" class="w-full btn_login text-base py-4">
                        绑定手机号
                    </button>
                </div>
            </form>
            ...
            <section class="form w-[381px]">
                <div class="bg-[#13151b] my-5   formDiv relative  flex items-center mx-auto  h-14">
                    <div class=" border-none w-full h-full text-sm flex justify-center items-center relative">
                        ...
                        <input v-model="phonenum" type="tel" autocomplete="off" class=" input-name text-[#d0d0d0]   
                ...
                <section class="btn py-5">
                    <div @click="binePhone" class="py-3 cursor-pointer text-base w-full btn_login text-center">
                        提交验证
                    </div>
                </section>
            </section>
...
function getCode() {
    ...
    // if(selectCode.value == 95 && (!phonenum.value?.match(/^\d{10}$/))) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //myanmar
    // if(selectCode.value == 95 && (!phonenum.value?.match(/^9(2|4|5|6|7|8|9|)\d{8}$/))) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 })
   
    // if (selectCode.value == 852 && !/^(\+?852\-?)?[2356789]\d{3}\-?\d{4}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //Hong Kong
    
    // if (selectCode.value == 853 && !/^(\+?853\-?)?[68]\d{3}\-?\d{4}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 })//Macao
    // if (selectCode.value == 886 && !/^(\+?886\-?|0)?9\d{8}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 })//Taiwan
    // if (selectCode.value == 971 && /^(?:00971|\+971|0)?(?:50|51|52|55|56|58|2|3|4|6|7|9)\d{7}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 })//United Arab Emirates


    //  if (selectCode.value == 81 && !/^(0([1-9]{1}-?[1-9]\d{3}|[1-9]{2}-?\d{3}|[1-9]{2}\d{1}-?\d{2}|[1-9]{2}\d{2}-?\d{1})-?\d{4}|0[789]0-?\d{4}-?\d{4}|050-?\d{4}-?\d{4})$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //japan
    //  if (selectCode.value == 63 && !/^(\+?63|0)?[2-9]\d{2}\d{3}\d{4}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //phili
    //  if (selectCode.value == 66 && !/^(\+?66|0)-?[1-9]\d{7}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //thai
    //  if (selectCode.value == 65 && !/^(\+?65)?[689]\d{7}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //singapore
    //  if (selectCode.value == 84 && !/^(84|0[3|5|7|8|9])+([0-9]{8})\b/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //vietnam
    //  if (selectCode.value == 60 && !/^(01[0-46-9]-*[0-9]{7,8}|011-*([0-9]{8}|[0-9]{7}))$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //malay
    //  if (selectCode.value == 62 && !/^(^\+62|0)(2[12]|5[2-9]|7[78]|8[1-9]|9[1-9])[0-9]{6,9}$/.test(phonenum.value))return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //indo
    //  if (selectCode.value == 855 && !/^\+?(855)?(1|9|8)[0-9]{7,8}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //cambodia
    //  if (selectCode.value == 856 && !/^\+?(856-?|0)(20|21|30|31|71|81|91)[0-9]{6,7}$/.test(phonenum.value)) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 }) //laos

    //  if(selectCode.value == 86 &&  (!/^((1[3,5,8,7,9][0-9])|(14[5,7])|(17[0,6,7,8])|(19[7]))\d{8}$/.test(phonenum.value ))) return instance.proxy.$message({ message: '手机号码格式不对', duration: 2 })//china
    getValidStr()
}
