<template>
    <div class="w-full h-screen bg-[#171a21]   relative sm:flex-auto sm:mx-[19px] sm:bg-[#1f2129]">
        <div
      class="h-[60px]  w-full text-gray-400 sm:bg-[#1f2129]    px-2 flex justify-between sm:justify-center items-center">
      <ChevronLeftIcon @click="goUser()" class="w-7 h-7  sm:hidden "></ChevronLeftIcon>
      <p class=" text-[17px] sm:text-center mr-5 sm:mr-0"> 反馈 </p>
      <p></p>
    </div>

   <section class="w-full text-[#7e7e7e] text-[18px] tracking-wider pt-5">
     <div class="py-2">
        <div @click="feedbackDialog = true" class="px-3 py-5 bg-[#13151b] flex items-center justify-between">
     ...
     <div class="py-1">
        <div class="px-3 py-3 bg-[#13151b] w-full flex items-center justify-between">
            <form @click.prevent class="w-full ">
                <input v-model="phone" max="11" min="11" maxlength="11" type="tel" placeholder="手机号码" class="
                w-full bg-transparent input-name border-none outline-none placeholder:text-[#7e7e7e] placeholder:text-[18px]">
            </form>
        </div>
     </div>
     <div class="pt-1">
     ...
     <div class="px-3  w-full bg-[#13151b]">
        <textarea v-model="textarea" placeholder="请简要输入您的问题和建议"  class="  w-full bg-transparent h-32 px-4   outline-none border-none input-name placeholder:text-[#7e7e7e] placeholder:text-[18px]"></textarea>
     </div>
   ...
   <div v-if="feedbackDialog" class="absolute z-10 bottom-[0px] w-full overflow-y-hidden  h-full bg-[rgba(0,0,0,0.72)]">
    <div class="px-5 w-full fixed h-[screen] bottom-0 z-20  bg-[rgba(0,0,0,0.72)]">
            ...
            items-center text-center text-[18px] text-[#d0d0d0] justify-center py-4 bg-[#272c33] px-2 w-full">
            {{tet.title}}
         </div>
         <div @click="feedbackDialog = false" class="flex items-center py-3 my-3 text-center text-[18px] justify-center bg-[#272c33] px-2 w-full">
            取消
         </div>
          </div>
   </div>
