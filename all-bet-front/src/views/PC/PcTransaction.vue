<template>
  <div class="hidden sm:block relative flex-auto mx-[19px] sm:h-auto">
    ...
    <div class="w-full tabs flex items-center justify-between border-b border-gray-800">
      <div
        @click="Tabclick(0)"
        class="text-base text-center py-2 cursor-pointer w-[120px] tracking-wider"
      ...
      <div
        @click="Tabclick(4)"
        class="text-base text-center py-2 cursor-pointer w-[120px] tracking-wider"
        :class="activeTab == 4 ? 'border-b border-[#eed0a4]' : 'text-[#a6a6a6]'"
      >
        游戏记录
      </div>
      <div
        @click="Tabclick(5)"
        class="text-base text-center py-2 cursor-pointer w-[120px] tracking-wider"
        :class="activeTab == 5 ? 'border-b border-[#eed0a4]' : 'text-[#a6a6a6]'"
      >
        日最大充值
      </div>
    </div>
    ...
    <section class="my-2 w-full bg-[#1f2129] flex items-center justify-between">
      ...
      <div class="flex items-center space-x-2 px-5">
        <div
          @click="request"
          class="text-center cursor-pointer w-[109px] h-[42px] flex justify-center items-center border border-[#c15410] text-white"
        >
          查询
        </div>
    ...
    <section
      ...
      class="mt-2 w-full bg-[#1f2129] h-[755px]"
    >
      <div class="overflow-auto h-[650px]">
        <table class="w-full min-w-full text-[#a6a6a6]">
          <thead class="border-b border-[#333]">
            <tr>
              <th scope="col" class="text-[15px] font-medium w-[150px] py-4">存款时间</th>
              <th scope="col" class="text-[15px] font-medium w-[180px] py-4">订单号</th>
              <th scope="col" class="text-[15px] font-medium py-4">存款类型</th>
              <th scope="col" class="text-[15px] font-medium py-4">存款状态</th>
              <th scope="col" class="text-[15px] font-medium py-4">金额 | 明细</th>
          ...
          <tbody>
            ...
            >
              ...
              <td
                class="text-[13px] font-medium text-center py-2"
                :class="
                  record?.status == 0
                    ? 'text-red-500'
                    : record?.status == 1
              ...
              <td class="text-[13px] font-medium text-center py-2"></td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <section v-if="activeTab == 2" class="mt-2 w-full bg-[#1f2129] h-[755px]">
      <div class="overflow-auto h-[650px]">
        <table class="w-full min-w-full text-[#a6a6a6]">
          <thead class="border-b border-[#333]">
            <tr>
              <th scope="col" class="text-[15px] font-medium w-[150px] py-4">时间</th>
              <th scope="col" class="text-[15px] font-medium w-[180px] py-4">用户名</th>
              <th scope="col" class="text-[15px] font-medium py-4">类型</th>
              <th scope="col" class="text-[15px] font-medium py-4">状态</th>
              <th scope="col" class="text-[15px] font-medium py-4">金额 | 明细</th>
    ...
    <section v-if="activeTab == 4" class="mt-2 w-full bg-[#1f2129] h-[755px] ">
      <div class="overflow-auto h-[650px]">
        <div v-if="betRecordData?.count" class="my-2  w-full min-w-full border-b border-[#494646] flex justify-between items-center px-10 py-2">
        ...
        <table class="w-full min-w-full text-[#a6a6a6]">
          <thead class="border-b border-[#333]">
            <tr>
              <th scope="col" class="text-[15px] font-medium w-[150px] py-4">下注时间</th>
              <th scope="col" class="text-[15px] font-medium w-[180px] py-4">下注金额</th>
              <th scope="col" class="text-[15px] font-medium py-4">币种</th>
              <th scope="col" class="text-[15px] font-medium py-4">游戏结果</th>
              <th scope="col" class="text-[15px] font-medium py-4">游戏类型</th>
              <th scope="col" class="text-[15px] font-medium py-4">桌台号</th>
              <!-- <th scope="col" class="text-[15px] font-medium   py-4 ">
                                操作
                            </th> -->
            </tr>
            ...
            >
              ...
              <td class="text-[13px] font-medium text-center py-2">
                ...
                <span
                  ...
                  >{{ bet?.gameResult }}</span
                >
                <span  v-else>{{ TranResult(bet) }}</span>
              </td>
    ...
    <section v-if="activeTab == 5" class="mt-2 w-full bg-[#1f2129] h-[755px]">
      <div class="overflow-auto h-[650px]">
        <table class="w-full min-w-full text-[#a6a6a6]">
          <thead class="border-b border-[#333]">
            <tr>
              <th scope="col" class="text-[15px] font-medium w-[150px] py-4">时间</th>
              <th scope="col" class="text-[15px] font-medium w-[180px] py-4">金额</th>
              <th scope="col" class="text-[15px] font-medium py-4">币种</th>
            </tr>
          </thead>
          ...
          <tbody>
            <tr
              v-for="record in maxDepoRecordData"
              :key="record.id"
              class="border-b border-[#333]"
            >
              <td class="text-[13px] font-medium w-[150px] text-center py-3">
    ...
    <section v-if="activeTab == 3" class="mt-2 w-full bg-[#1f2129] h-[755px]"></section>
  </div>
</template>
...
const minDate = () => {
  var d = moment().subtract(30, "d").format("YYYY-MM-DD");
  return d;
};
...
const WashRecord = () => {
  ...
  allApi
    .GetWashRecord({ data: data, key: userValue?.value?.key })
    .then((res) => {
      Loading.hideLoading();
      if (res.data.code == "1") {
...
const BetRecord = () => {
  ...
  allApi
    .GetBetRecord({ data: data, key: userValue?.value?.key })
    .then((res) => {
      Loading.hideLoading();
      if (res.data.code == "1") {
